...
void Armas_HUD::render()
{
	...
	if (mainWeapon != nullptr) {
		...
		Vector2D pos1 = Vector2D((int)((App::camera.w - size1.getX()) * 0.03), (int)((App::camera.h - size1.getY()) * 0.8));
		...
		if (secondWeapon != nullptr) {
			...
			Vector2D pos2 = Vector2D((int)((App::camera.w - size2.getX()) * 0.15), (int)((App::camera.h - size2.getY()) * 0.8));
			...
			if (currentSelectedWeapon) {
				...
				pos1 = Vector2D((int)((App::camera.w - size1.getX()) * 0.02), (int)((App::camera.h - size1.getY()) * 0.8));
